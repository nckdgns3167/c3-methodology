...
6. 구현
7. `init.sh` 통과 확인 → `git commit` (메시지에 feature ID 포함)
8. **plan Post-mortem 섹션 작성** (feature `done` 처리 _전_, ADR-0013) — 추정 vs 실제 / 잘된 것 / 어려웠던 것 / 다음 feature에 가져갈 것 / 백로그 영향. 5~10줄. `validate-feature-list.ts`가 done 시 비어있지 않음 검증.
9. `feature_list.json`의 `status`를 `in_progress` → `done` 처리 + 두 번째 commit (메타 type `chore:` 권장)
