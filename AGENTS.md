...
| **`docs/plans/`**       | _실행 계획_ 영구화 (Plan-as-Code, ADR-0010). frontmatter + Pre-mortem + Post-mortem 강제, supersede 체인 |
| **`docs/postmortems/`** | _사건 회고_ (Blameless, 5 Whys, action items 백로그 통합 — ADR-0013). `YYYY-MM-DD-<title>.md`            |
...
- ADR-0013 — Retro & Postmortem 정책 (Plan-as-Code 통합, Pre-mortem ↔ Post-mortem 페어링)
- ADR-0014 — Multi-agent Plan Cross-Review (Claude × Codex, risk_class 분류 + GitHub PR @codex review)
- ADR-0015 — 제품 자유도 단계화 + 디자인 시스템 구조 분리 (Phase 1/2/3, 컴포넌트 스타일 ↔ 디자인 시스템, F-015/F-035 territory)
- ADR-0016 — Free/Pro/Team 비즈니스 모델 + 다중 포폴 (Phase 1.5 polish, 유료화 트리거, F-100 활성 시 plan_limit hard)
...
5. **plan frontmatter `risk_class` 분류** (ADR-0014 — critical/mid/easy) + critical/mid 시 PR + `@codex review` → plan 안 `## Cross-review (Codex)` 섹션에 결과 박음 (Convergence/Divergence/Codex 단독 catch). easy는 skip. 가이드: `docs/cross-review-guide.md`.
6. `acceptance.verifiable.tests`에 명시된 테스트 파일 **먼저 작성** (TDD 강제)
7. 구현
8. `init.sh` 통과 확인 → `git commit` (메시지에 feature ID 포함)
9. **plan Post-mortem 섹션 작성** (feature `done` 처리 _전_, ADR-0013) — 추정 vs 실제 / 잘된 것 / 어려웠던 것 / 다음 feature에 가져갈 것 / 백로그 영향. 5~10줄. `validate-feature-list.ts`가 done 시 비어있지 않음 검증.
10. `feature_list.json`의 `status`를 `in_progress` → `done` 처리 + 두 번째 commit (메타 type `chore:` 권장)
...
### 6.1 컴포넌트 작업 시작 전 필수 단계 (AI 슬롭 회피)

영상 _"AI로 만든 제품이 안 팔리는 이유"_ 인사이트 + ADR-0009/0011/0012 정합. AI가 컨텍스트 길어지면 DESIGN.md 까먹는 것을 강제로 차단.

1. **DESIGN.md 현재 값 재확인 (1~5분)**: 작업하려는 토큰(color/space/font/radius)이 DESIGN.md에 박혔는지 확인. 변경 없으면 fast skip. 변경/추가 필요 시 DESIGN.md 먼저 갱신 → F-007 build-tokens 재실행.
2. **F-008 (Hero 앵커) 작업의 경우 추가 — 벤치마킹 워크플로** (ADR-0012):
   - 토스/리니어/원티드 몽타주 1~2 reference 선택
   - 스크린샷 또는 URL을 AI에 던져 클론 분석 또는 design.md 추출
   - 추출된 값을 DESIGN.md 1차 박음 (typography는 ADR-0011 한국어 best practice 우선)
   - 우리만의 색깔(채용·경력 톤, accent color) 추가
   - 시간 박스 1d 이하 — 초과 시 자체 진행
3. **외부 reference 사용 시 라이선스 검증** (ADR-0012): 상업 이용 + 수정 자유만. 사용 명시는 plan 또는 commit message에.
4. **한국어 타이포 best practice 강제** (ADR-0011): 자간 `-0.02em` / 행간 heading `1.3~1.4` / body `1.5~1.6` / Pretendard 폰트 1차. 위반은 Stylelint A3 검사로 차단.
5. **Territory 분리 강제** (ADR-0015): _C3 에디터 자체_ 시각 SSOT는 `DESIGN.md` (ADR-0009). _사용자 페이지_ 디자인은 `design_systems` 테이블 + `page.theme_override_json` (F-035 작업 시점 박힘). DESIGN.md에 사용자 페이지 디자인 박지 _마_, design*systems 테이블에 C3 에디터 디자인 박지 *마\_. F-015 (인스펙터)는 page meta theme override만 — design system 저장/불러오기는 F-035 영역.
...
9. **사건 발생 시 `docs/postmortems/<날짜>-<title>.md` 작성** (ADR-0013) — Blameless 원칙 (사람 X, 시스템 O), 5 Whys로 시스템 원인까지, action items는 _반드시_ `feature_list.json`에 새 F-XXX로 매핑. 사소한 디버깅은 `claude-progress.md`에 한두 줄로 충분. 경계: "다른 사람(또는 6개월 뒤의 나)이 같은 문제 겪을 가능성 있는가?" yes면 정식 postmortem.
